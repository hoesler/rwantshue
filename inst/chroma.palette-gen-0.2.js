...
var paletteGenerator = (function(undefined){
	...
	ns.generate = function(colorsCount, checkColor, forceMode, quality, ultra_precision, distanceType, rng_fun){
		...
		if(forceMode){
			...
			for(i=0; i<colorsCount; i++){
				...
				var color = [100*rng_fun(),100*(2*rng_fun()-1),100*(2*rng_fun()-1)];
				while(!checkLab(color)){
					color = [100*rng_fun(),100*(2*rng_fun()-1),100*(2*rng_fun()-1)];
				}
				colors.push(color);
			}
				...
				for(i=0; i<colors.length; i++){
					...
					for(j=0; j<i; j++){
						...
						if(d>0){
							...
							// Jitter
							vectors[j].dl += 2 - 4 * rng_fun();
							vectors[j].da += 2 - 4 * rng_fun();
							vectors[j].db += 2 - 4 * rng_fun();
						}
					}
				}
				// Apply Force
			...
			var kMeans = [];
			for(i=0; i<colorsCount; i++){
				var lab = [100*rng_fun(),100*(2*rng_fun()-1),100*(2*rng_fun()-1)];
				
				var max_iter = 100000;
				for(j=0; j<max_iter; j++){
					lab = [100*rng_fun(),100*(2*rng_fun()-1),100*(2*rng_fun()-1)];
					if(checkColor2(lab)){
						kMeans.push(lab);
						break;
					}
				...
				if (j==max_iter) {
					console.log("No valid color after " + max_iter + "iterations")
				}
			}
